...
const TreeItem: React.FC<TreeItemProps> = ({
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    ...
    const isLorebookDir = node.name === 'lorebook' || node.mapping?.field === 'globalLore'
  ...
  const canDrag = node.mapping?.field === 'globalLore' && node.type === 'file'
  const canDrop = isDir && (node.name === 'lorebook' || node.mapping?.field === 'globalLore')
  ...
  return (
    <>
      ...
      >
        {isDir && (
          <span className={`re-tree-chevron${isExpanded ? ' open' : ''}`}>▶</span>
        )}
