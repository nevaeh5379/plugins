...
globalThis.__RISU_LOADER_HOOK__ = Object.freeze({
  ...
  ,sendMessage: async (text, options = {}) => {
    const input = Array.from(document.querySelectorAll('.default-chat-screen textarea.text-input-area'))
      .find((element) => element.isConnected);
    if (!input) throw new Error('Risu chat composer was not found.');
    const row = input.parentElement;
    const sendButton = row?.querySelector('button.button-icon-send');
