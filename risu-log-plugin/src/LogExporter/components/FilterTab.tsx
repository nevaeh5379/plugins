...
import { Tag, Checkbox, Divider, List, Typography } from 'antd';
  ...
  return (
    <div className="tab-content">
      <div className="tab-section">
        <Title level={5} className="tab-section-title">참가자 필터</Title>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          ...
          {participants.size === 0 && (
            <Text type="secondary" style={{ fontSize: '0.9em' }}>참가자 정보가 없습니다.</Text>
          )}
        </div>
      </div>
      ...
      {uiClasses.length > 0 && (
        <>
          <Divider />
          <div className="tab-section">
            <Title level={5} className="tab-section-title">UI 요소 필터</Title>
            <Text type="secondary" style={{ fontSize: '0.85em' }}>
              ...
              renderItem={classInfo => {
                ...
                return (
                  <List.Item style={{ padding: '8px 12px', borderBottom: '1px solid var(--border-color-light)' }}>
                    ...
                    >
                      <span style={{ fontSize: '0.95em', color: 'var(--text-primary)' }}>{classInfo.displayName}</span>
                    </Checkbox>
                  </List.Item>
                );
              }}
